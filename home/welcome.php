    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    $result = mysqli_query($db_connection,"SELECT * FROM quiz ORDER BY nama_quiz DESC") or die('Error');
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        $sahi = $row['benar'];
                        $eid = $row['idq'];
                    $q12=mysqli_query($db_connection,"SELECT score FROM history WHERE idq='$eid' AND email='$email'" )or die('Error98');
                ...
                <?php
                    ...
                    {
                        $eid=@$_GET['eid'];
                        $q=mysqli_query($db_connection,"SELECT * FROM history WHERE idq='$eid' AND email='$email' " )or die('Error157');
                ...
                <?php
                    if(@$_GET['q']== 2) 
                    {
                        $q=mysqli_query($db_connection,"SELECT * FROM history WHERE email='$email' ORDER BY date DESC " )or die('Error197');
                        echo  '<div class="panel title">
                        <table class="table table-striped title1" >
                        <tr style="color:black;"><td><center><b>S.N.</b></center></td><td><center><b>Quiz</b></center></td><td><center><b>Question Solved</b></center></td><td><center><b>Right</b></center></td><td><center><b>salah<b></center></td><td><center><b>Score</b></center></td>';
                        $c=0;
                        while($row=mysqli_fetch_array($q) )
                        {
                        $eid=$row['idq'];
                        ...
                        $r=$row['benar'];
                        $qa=$row['level'];
                        $q23=mysqli_query($db_connection,"SELECT nama_quiz FROM quiz WHERE  idq='$eid' " )or die('Error208');
                    ...
                    if(@$_GET['q']== 3) 
                    {
                        $q=mysqli_query($db_connection,"SELECT * FROM rank ORDER BY score DESC " )or die('Error223');
                        ...
                        while($row=mysqli_fetch_array($q) )
                        {
                            $e=$row['email'];
                            $s=$row['score'];
                            $q12=mysqli_query($db_connection,"SELECT * FROM users WHERE email='$e' " )or die('Error231');
